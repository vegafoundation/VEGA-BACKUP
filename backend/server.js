...
app.post('/api/admin/login', async (req, res) => {
  ...
  if (!safetyCheck.passed) {
    return res.status(403).json({ success: false, resonanceScore: safetyCheck.score, resonanceMessage: safetyCheck.message });
  }
  const authResult = await validateCredentials(null, password);
  ...
  res.json({ 
    ...
    resonanceScore: safetyCheck.score, 
    resonanceMessage: safetyCheck.message 
  });
});
